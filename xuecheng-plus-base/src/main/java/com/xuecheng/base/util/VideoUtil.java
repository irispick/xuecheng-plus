...
public class VideoUtil {
    ...
    public Boolean check_video_time(String source,String target) {
        ...
        int source_second = get_video_second(source_time);
        int target_second = get_video_second(target_time);
        if ((source_second - target_second) <= 2) {
            return true;
        }
        return false;
    }
    ...
    // 将hh:MM:ss转换为秒
    public int get_video_second(String video_time) {
        String[] times = video_time.split(":");
        int[] timeList = new int[times.length];
        for (int i = 0; i < times.length; i++) {
            timeList[i] = Integer.parseInt(times[i]);
        }
